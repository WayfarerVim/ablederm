  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#F9F7F4', fontFamily: '"DM Sans", -apple-system, sans-serif', color: '#2C2C2C' }}>
      ...
      <section
        ...
        style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative', overflow: 'hidden', paddingTop: 72, backgroundImage: 'url(/ablederm-banner.png)' }}
      >
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)' }} />
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 720, margin: '0 auto', padding: '60px 24px', textAlign: 'center', color: '#fff' }}>
          <h1 className="serif fade-up-delay-1" style={{ fontSize: 'clamp(36px, 5vw, 68px)', fontWeight: 300, lineHeight: 1.1, margin: '0 0 8px' }}>
            Precision Dermatology
          </h1>
          <h1 className="serif fade-up-delay-1" style={{ fontSize: 'clamp(36px, 5vw, 68px)', fontWeight: 500, lineHeight: 1.1, margin: '0 0 32px', fontStyle: 'italic' }}>
            and Aesthetic Medicine
          </h1>
          <p className="fade-up-delay-2" style={{ fontSize: 15, lineHeight: 1.8, maxWidth: 560, margin: '0 auto 40px', fontWeight: 300 }}>
            Evidence-based dermatology and aesthetic medicine delivered by Dr Anupama Lochab, GPwSI Dermatology. Precision diagnostics and genuinely personalised care at Bradstowe Surgery, Woking.
          </p>
          <div className="fade-up-delay-3" style={{ display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
            <a
              href="#contact"
              style={{ background: '#fff', color: '#0d9488', padding: '12px 28px', fontWeight: 500, fontSize: 14, letterSpacing: '0.04em', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8 }}
            >
              Book Consultation
            </a>
            <a
              href="#services"
              style={{ border: '1px solid #fff', color: '#fff', padding: '12px 28px', fontWeight: 500, fontSize: 14, letterSpacing: '0.04em', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8 }}
            >
              Explore Services
            </a>
          </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 80, alignItems: 'start' }} className="conditions-grid">
            <div>
              ...
              <a href="#contact" className="btn-primary" style={{ display: 'inline-flex' }}>Enquire Now <ChevronRight size={14} /></a>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {conditions.map(c => <div key={c} className="condition-tag">{c}</div>)}
            </div>
          </div>
        </div>
      </section>
            ...
            {pricingTiers.map(tier => (
              <div key={tier.name} className={`pricing-card ${tier.highlight ? 'featured' : ''}`}>
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 32px' }}>
                  {tier.features.map(f => (
                    <li key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', fontSize: 13, color: tier.highlight ? 'rgba(249,247,244,0.85)' : '#6B6060', borderBottom: `1px solid ${tier.highlight ? 'rgba(249,247,244,0.08)' : 'rgba(107,62,62,0.08)'}`, fontWeight: 300 }}>
                      <CheckCircle2 size={14} style={{ color: '#D4AF37', flexShrink: 0 }} />{f}
                    </li>
                  ))}
                </ul>
                <a href="#contact" style={{ display: 'block', textAlign: 'center', padding: '13px', border: `1px solid ${tier.highlight ? 'rgba(212,175,55,0.5)' : 'rgba(107,62,62,0.25)'}`, color: tier.highlight ? '#D4AF37' : '#6B3E3E', textDecoration: 'none', fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, transition: 'all 0.3s' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'rgba(212,175,55,0.1)'; }}
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64 }} className="contact-grid">
          ...
          <div style={{ background: 'rgba(249,247,244,0.05)', border: '1px solid rgba(249,247,244,0.08)', padding: '40px 32px' }}>
            ...
            <button className="btn-primary" style={{ width: '100%', justifyContent: 'center', background: '#D4AF37', color: '#3D2626' }}
              onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#E8CB6A'; }}
